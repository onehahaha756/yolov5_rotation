...
class ComputeLoss_cls:
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLoss_cls, self).__init__()
        ...
        self.BCEPactchCls = BCEPactchCls
        # import pdb;pdb.set_trace()
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.nc > 1:  # cls loss (only if multiple classes)
                    t = torch.full_like(
                        ps[:, 6:], self.cn, device=device)  # targets
    ...
    def build_targets(self, p, targets):
        ...
        bs = p[0].shape[0]
        patch_cls = torch.zeros(bs, 1, device=targets.device)
        patch_cls[targets[:, 0].long()] = 1
        # import pdb;pdb.set_trace()
        ...
        for i in range(self.nl):
            ...
            tcls.append(c)  # class
        # import pdb;pdb.set_trace()
        return tcls, tbox, indices, anch, patch_cls
