...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if nL:
            ...
            #labels[:,[3,4]] /= max(img.shape[0],img.shape[1])
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        #print('resized',self.img_hw[index])
...
def random_perspective_rotation(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates_rotation(
            box1=targets[:, 1:5].T * s, box2=new.T, width=width, height=height, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]
        targets[:, 1:5] = new[i]
    # print('after',targets)
    return img, targets
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            xy = np.ones((n * 4, 3))
            xy[:, :2] = targets2[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(
                n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy = xy @ M.T  # transform
            xy = (xy[:, :2] / xy[:, 2:3] if perspective else xy[:, :2]
                  ).reshape(n, 8)  # perspective rescale or affine
...
# box1(4,n), box2(4,n)
def box_candidates_rotation(box1, box2, width, height, wh_thr=2, ar_thr=20, area_thr=0.1, eps=1e-16):
    # Compute candidate boxes: box1 before augment, box2 after augment, wh_thr (pixels), aspect_ratio_thr, area_ratio
    w1, h1 = box1[2] - box1[0], box1[3] - box1[1]
    w2, h2 = box2[2] - box2[0], box2[3] - box2[1]
    # import pdb;pdb.set_trace()
    ...
    # candidates
    return save_box & (w2 > wh_thr) & (h2 > wh_thr) & (w2 * h2 / (w1 * h1 + eps) > area_thr) & (ar < ar_thr)
...
def verify_image_label_rotation(params):
...
def dataset_stats(path='coco128.yaml', autodownload=False, verbose=False):
    ...
    for split in 'train', 'val', 'test':
        ...
        dataset = LoadImagesAndLabels(
            data[split], augment=False, rect=True)  # load dataset
        if split == 'train':
