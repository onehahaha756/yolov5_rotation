...
def xywhn2xyxy(x, w=640, h=640, padw=0, padh=0):
    ...
    #to avoid w(or h ) bigger than 1., rotation normalized w,h is compute by (w,h)/max(img.shape[:2])
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = w * x[:, 0] - max(w,h)*x[:, 2] / 2 + padw  # top left x
    y[:, 1] = h * x[:, 1] - max(w,h)*x[:, 3] / 2 + padh  # top left y
    y[:, 2] =  w * x[:, 0] + max(w,h)*x[:, 2] / 2 + padw  # bottom right x
    y[:, 3] =  h * x[:, 1] + max(w,h)*x[:, 3] / 2 + padh  # bottom right y
    #y[:,4]=x[:,4]
    return y
